...
const navItems = [
  ['Home', '#top'], ['About Us', '#about'], ['Results', '#results'], ['Faculty', '#faculty'],
...
function Intro() {
  return <section id="about" className="relative overflow-hidden py-5 md:py-7"><div className="absolute left-0 top-0 h-16 w-16 border-l-[3px] border-t-[3px] border-[#8E140E] opacity-70" /><div className="container-wide grid gap-7 md:grid-cols-[1fr_1fr] md:items-center">
    ...
    <div className="reveal relative mx-auto w-full max-w-[440px] overflow-hidden rounded-3xl border-[6px] border-white bg-white shadow-[0_10px_26px_rgba(31,40,56,.18)] ring-1 ring-[#1C2A37]/25"><img src="/campus-courtyard.jpg" alt="Students at Vivekananda Concept School" className="block aspect-[4/3] w-full rounded-2xl object-cover" data-testid="img-about" /></div>
  </div></section>;
}
...
function Facilities() {
  return <section id="media" className="py-6 md:py-9"><div className="container-wide"><Heading title="Our" accent="Facilities" /><div className="mx-auto mt-8 grid max-w-[880px] grid-cols-1 gap-x-12 gap-y-10 sm:grid-cols-2">{facilities.map(([Icon, title, copy], index) => <article key={title} className="reveal flex flex-col items-center text-center" data-testid={`card-facility-${index + 1}`}><span className={`facility-circle grid h-32 w-32 place-items-center rounded-full ${index % 2 === 0 ? 'text-[#8E140E]' : 'text-[#1F2838]'}`}><Icon size={54} /></span><h3 className="mt-4 font-sans text-[24px] font-medium text-[#1F2838]">{title}</h3><p className="mt-2 max-w-[320px] text-[16px] leading-6 text-[#1F2838]/75">{copy}</p></article>)}</div></div></section>;
}
...
function Testimonials() {
  ...
  return <section id="blogs" className="py-5 md:py-7"><div className="container-wide"><Heading title="Parent Say" accent="About us" /><div className="relative mx-auto mt-6 max-w-[760px] px-10 text-center sm:px-14" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={() => setPaused(false)} aria-roledescription="carousel" aria-live="polite"><Quote className="absolute left-0 top-0 text-[#8E140E]" fill="currentColor" size={40} /><Quote className="absolute right-0 top-0 rotate-180 text-[#8E140E]" fill="currentColor" size={40} /><div key={active} className="testimonial-fade"><blockquote className="whitespace-pre-line text-[16px] leading-[1.55] text-[#1F2838]" data-testid="text-testimonial">{testimonial.quote}</blockquote><p className="mt-4 text-right text-[18px] font-semibold text-[#8E140E]" data-testid="text-testimonial-name">{testimonial.name}</p></div><div className="mt-5 flex justify-center gap-1.5">{testimonials.map((item, index) => <button key={item.name} onClick={() => setActive(index)} className={`h-1.5 ${active === index ? 'w-6 bg-[#8E140E]' : 'w-5 bg-[#1F2838]'}`} aria-label={`Show testimonial ${index + 1}`} data-testid={`button-testimonial-dot-${index + 1}`} />)}</div><div className="mt-3 flex justify-center gap-2"><button onClick={() => setActive((active + testimonials.length - 1) % testimonials.length)} className="rounded p-1 text-[#8E140E]" aria-label="Previous testimonial" data-testid="button-testimonial-previous"><ArrowLeft size={16} /></button><button onClick={() => setActive((active + 1) % testimonials.length)} className="rounded p-1 text-[#8E140E]" aria-label="Next testimonial" data-testid="button-testimonial-next"><ArrowRight size={16} /></button></div><div className="absolute -right-2 bottom-4 hidden h-20 w-14 rounded-[50%] bg-gradient-to-br from-[#8E140E] via-white to-[#1F2838] md:block" /></div></div></section>;
}
...
function CallFab({ onEnquire }: { onEnquire: () => void }) {
  return <div className="fixed bottom-5 right-5 z-40 flex flex-col items-center gap-1 sm:bottom-7 sm:right-7">
    <button onClick={onEnquire} className="call-cloud relative block h-[76px] w-[170px]" aria-label="Start your admission enquiry" data-testid="button-call-cloud">
      ...
      <span className="relative flex h-full w-full flex-col items-center justify-center pb-3 text-[12px] font-bold leading-[1.2] tracking-[.09em] text-[#1C2A37]"><span>ADMISSION</span><span>ENQUIRY</span></span>
    </button>
    <button onClick={onEnquire} className="relative grid h-[74px] w-[74px] shrink-0 place-items-center rounded-full border-[5px] border-white bg-[#1C2A37] text-white shadow-[0_10px_24px_rgba(28,42,55,.45)] ring-2 ring-[#1C2A37]" aria-label="Start your admission enquiry" data-testid="button-call-fab">
      <span className="call-fab-ripple" aria-hidden="true" />
      <span className="call-fab-ripple" style={{ animationDelay: '1.1s' }} aria-hidden="true" />
      <Phone className="call-fab-icon relative" size={32} fill="currentColor" />
    </button>
  </div>;
}
...
function Home() {
  ...
  useEffect(() => { document.title = 'Vivekananda Concept School | Pulivendla'; const description = 'Vivekananda Concept School in Pulivendla offers thoughtful education from Pre-School through High-School.'; const set = (name: string, content: string) => { let meta = document.querySelector(`meta[name="${name}"]`); if (!meta) { meta = document.createElement('meta'); meta.setAttribute('name', name); document.head.appendChild(meta); } meta.setAttribute('content', content); }; set('description', description); }, []);
  return <div className="grain relative min-h-[100dvh] overflow-hidden bg-white">
    ...
    <WallpaperLayer />
    <div className="relative z-10"><Header onEnquire={() => setModal(true)} /><Hero /><Intro /><Results /><Facilities /><Faculty /><Gallery /><Testimonials /><Admissions onEnquire={() => setModal(true)} /><Footer onEnquire={() => setModal(true)} /></div>
    <CallFab onEnquire={() => setModal(true)} />
